...
internal sealed class DependencyInjectionEventSource : EventSource
{
    ...
    private readonly List<WeakReference<MutableServiceProvider>> _providers = new();
    ...
    [NonEvent]
    public void ServiceResolved(MutableServiceProvider provider, Type serviceType)
    {
    ...
    [NonEvent]
    public void CallSiteBuilt(MutableServiceProvider provider, Type serviceType, ServiceCallSite callSite)
    {
        if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
        {
    ...
    [NonEvent]
    public void DynamicMethodBuilt(MutableServiceProvider provider, Type serviceType, int methodSize)
    {
        if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
        {
    ...
    [NonEvent]
    public void ServiceProviderBuilt(MutableServiceProvider provider)
    {
        lock (_providers)
        {
            _providers.Add(new WeakReference<MutableServiceProvider>(provider));
        }
    ...
    [NonEvent]
    public void ServiceProviderDisposed(MutableServiceProvider provider)
    {
        lock (_providers)
        {
    ...
    [NonEvent]
    private void WriteServiceProviderBuilt(MutableServiceProvider provider)
    {
        if (IsEnabled(EventLevel.Informational, Keywords.ServiceProviderInitialized))
        {
            ...
            foreach (var collection in provider.CallSiteFactory.ServiceCollections)
            {
...
internal static class DependencyInjectionEventSourceExtensions
{
    ...
    public static void ExpressionTreeGenerated(this DependencyInjectionEventSource source, MutableServiceProvider provider, Type serviceType, Expression expression)
    {
