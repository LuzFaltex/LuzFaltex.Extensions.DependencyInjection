...
internal sealed class CallSiteFactory : IServiceProviderIsService
{
    ...
    internal void AddServiceCollection(string name, IServiceCollection serviceCollection)
    {
        _serviceCollections.Add(name, serviceCollection);
        Populate();
    }

    internal void RemoveServiceCollection(string name)
    {
        _ = _serviceCollections.TryGetValue(name, out var collection);
        Unpopulate(collection);
        _ = _serviceCollections.Remove(name);
    }

    internal bool IsDefaultServiceCollectionAdded()
    {
        var result = _serviceCollections.TryGetValue("Default", out _);
        if (result)
        {
            DefaultCollectionChecked = true;
        }

        return !DefaultCollectionChecked && result; 
    }

    internal List<IServiceCollection> ServiceCollections => _serviceCollections.Values.ToList();
        ...
        foreach (var collection in _serviceCollections.Values)
        {
            foreach (var descriptor in collection)
            {
                ...
                if (serviceType.IsGenericTypeDefinition)
                {
                    ...
                    if (MutableServiceProvider.VerifyOpenGenericServiceTrimmability)
                    {
    ...
    private void Unpopulate(IServiceCollection? collection)
    {
        foreach (var descriptor in collection!)
        {
            var serviceType = descriptor.ServiceType;
            var cacheKey = serviceType;
            _ = _descriptorLookup.Remove(cacheKey);
        }
    }
    ...
    internal ServiceCallSite? GetCallSite(Type serviceType, CallSiteChain callSiteChain) =>
        _callSiteCache.TryGetValue(new ServiceCacheKey(serviceType, DefaultSlot), out var site) ? site :
    ...
    public bool IsService(Type serviceType)
    {
        ...
        // If you update these make sure to also update the code in MutableServiceProvider.ctor
